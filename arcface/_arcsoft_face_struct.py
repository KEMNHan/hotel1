...
class Version(Structure):#SDK 版本信息
...
class ActiveFileInfo(Structure):#激活文件信息
...
class SingleFaceInfo(Structure):#单人脸信息
    rect: Rect  # 人脸框
    orient: c_int32  # 人脸角度
    _fields_ = [
        ('rect', Rect),
...
class MultiFaceInfo(Structure):#多人脸信息
    rects: Rect  # 人脸框数组
...
class FaceFeature(Structure):#人脸特征
...
class GenderInfo(Structure):#性别信息
...
class Angle3D(Structure):#3D 角度信息
    roll: c_float  # 横滚角
    yaw: c_float  # 偏航角
    pitch: c_float  # 俯仰角
...
class LivenessThreshold(Structure):#活体置信度
...
class LivenessInfo(Structure):#活体信息
